...
public class OAIService {
    ...
    public String analyzeFileName(String analyzableContent) {
        var result = this.openAIClient.chat().completions().create(ChatCompletionCreateParams.builder().addUserMessage(getFileNameAnalysisSystemPrompt()).addUserMessage(analyzableContent).model(configurationManager.getString(LLM_MODEL)).build());
        return result.choices().getFirst().message().content().orElseThrow(() -> new RuntimeException("Could not analyze file name"));
    }
